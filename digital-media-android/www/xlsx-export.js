...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetViews><sheetView rightToLeft="1" tabSelected="1" workbookViewId="0"><pane ySplit="1" topLeftCell="A2" activePane="bottomLeft" state="frozen"/></sheetView></sheetViews><sheetFormatPr defaultRowHeight="15"/>${cols}<sheetData>${body}</sheetData></worksheet>`;
  ...
  function download(filename, columns, rows, sheetName) {
    ...
    // نفس منفذ الحفظ الذي يستعمله باقي التصدير (انظر FBXSave في common.js)
    const save = (typeof window !== 'undefined' && window.FBXSave) || function (blob, fn) {
    ...
    save(build(columns, rows, sheetName), name);
  }
  ...
  function exportPosts(filename, posts, sheetName) {
    const columns = [
      ...
      { header: 'التصنيف النهائي', width: 14, type: 'text' },
      { header: 'مستوى الخطورة/الأهمية', width: 14, type: 'text' },
      { header: 'التصنيف الفرعي', width: 30, type: 'text' },
      { header: 'مرفوع العلم', width: 10, type: 'text' },
      { header: 'الإجراء الموصى به', width: 20, type: 'text' },
      { header: 'التحليل السياقي', width: 60, type: 'text' },
      { header: 'الاستناد القانوني والأخلاقي', width: 60, type: 'text' },
      { header: 'استثناء تم تطبيقه', width: 10, type: 'text' },
    ...
    const rows = posts.map((p, idx) => {
      ...
      return [
        ...
        a.classificationLabel || '', a.levelLabel || '', a.subCategory || '',
        a.flagged ? 'نعم' : 'لا', a.actionLabel || '',
        a.contextualAnalysis || '', a.legalBasis || '',
        a.exceptionApplied ? 'نعم' : 'لا',
