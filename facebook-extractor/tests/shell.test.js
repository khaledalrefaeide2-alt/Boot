...
test('الطبقة تحترم تفضيل تقليل الحركة فلا تدور حلقة', () => {
  ...
  assert.match(backdrop, /if\s*\(!raf\s*&&\s*!reduced\b/,
    'الحلقة مشروطة بغياب التفضيل لا بإيقافها بعد البدء');
});

test('الطبقة تتوقّف عن الحركة متى عجز الجهاز عن مجاراتها', () => {
  // قِيست نقرة تبديل المظهر عند 176ms — فوق سقف INP — لأن الصفحة تنتظر رسم
  // المستوي كاملاً. الحاكم يقيس الوسيط ويوقف الحلقة بدل أن يتلعثم بها.
  assert.match(backdrop, /const BUDGET\s*=\s*\d+/, 'ميزانية إطار مصرَّح بها');
  assert.match(backdrop, /spans\.sort/, 'القرار على الوسيط لا على إطار واحد شاذّ');
  assert.match(backdrop, /dataset\.static\s*=\s*'slow'/, 'الحالة تُعلَّم فلا تُستأنف الحلقة');
  assert.match(backdrop, /cv\.dataset\.static !== 'slow'/, 'play\\(\\) يحترم الحكم');
  // وحين نسكن، تبديل المظهر يحتاج رسمة يدوية إذ لا حلقة تلتقطه
  assert.match(backdrop, /if \(!raf\) draw1\(\)/, 'الرسم اليدوي مشروط بغياب الحلقة لا بسببه');
});
