...
(function (W) {
   ...
   * ============================================================ */
  // عنوان احتياطي يُستعمل فقط حين لا يضبط هذا المتصفح عنواناً خاصّاً به من
  // صفحة الإعدادات — حالة زائر لم يفتح الإعدادات قطّ، كمن يفتح رابط
  // results.html العام المنشور على استضافة ثابتة (GitHub Pages مثلاً).
  // املأه بعد نشر خادم local-server/server.js على استضافة عامة دائمة
  // التشغيل، ليرى كل زائر عام البيانات الحيّة بلا أي إعداد من طرفه.
  // إعداد صفحة الإعدادات على أيّ متصفّح يبقى له الأولوية دائماً.
  const DEFAULT_DB_URL = '';

  const db = {
    url: (localStorage.getItem(STORAGE.dbUrl) || DEFAULT_DB_URL || '').trim().replace(/\/+$/, ''),
    online: false
  };
